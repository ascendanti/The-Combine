...
| `agentica-prompts` | general | claude |  |
...
| `dead-code` | code | codex |  |
| `debug` | code | codex |  |
| `debug-hooks` | code | codex |  |
...
| `github-search` | research | claude |  |
| `graceful-degradation` | general | claude |  |
| `handoff` | memory | claude |  |
| `help` | general | claude |  |
...
| `morph-search` | research | claude |  |
| `mot` | general | claude |  |
| `nia-docs` | documentation | claude |  |
...
| `reference-sdk` | general | claude |  |
| `release` | general | claude |  |
| `remember` | general | claude |  |
| `repo-research-analyst` | research | claude |  |
| `repoprompt` | general | claude |  |
| `research` | research | claude |  |
| `research-agent` | research | claude |  |
| `research-external` | research | claude |  |
| `resume_handoff` | memory | claude |  |
| `review` | analysis | claude |  |
| `router-first-architecture` | planning | claude |  |
| `search-hierarchy` | research | claude |  |
| `search-router` | research | claude |  |
...
| Name | Description |
|------|-------------|
| `auto-cache-post` | Auto-Cache PostToolUse Hook
...
Calculates conte... |
| `braintrust_hooks` | Ensure state directories exist. |
| `cleanup-mcp` | SessionStart hook: Cleans up orphaned MCP node processes.
Prevents process accum... |
| `context-router` | Context Router - HOT/WARM/COLD context management

Implements progressive disclo... |
| `continuous-executor` | Stop Hook: Continuous Execution Architecture

This hook runs after every assista... |
| `continuous-learning-stop` | Continuous Learning Stop Hook
Extracts patterns from session and stores in memor... |
| `git-post-commit` | Git Post-Commit Hook - Historian Checkpoint
Automatically creates a checkpoint a... |
| `hook_launcher` | from __future__ import annotations

import json
import os
import shutil
import s... |
| `kg-context-gate` | KG Context Gate - PreToolUse Hook (Revised)

Based on claude-context-extender pa... |
| `kg-context-store` | import json
import sys
from pathlib import Path
from datetime import datetime

#... |
| `memory-recall-start` | Session start hook: Recall relevant memories.

Runs on session start to load con... |
| `memory-store-stop` | Session stop hook: Store session learnings.

Runs on session end to persist impo... |
| `module-reminder` | SessionStart Hook: Surface forgotten modules. |
| `n8n-notify` | n8n webhook notification hook for Claude Code.
Sends updates to an n8n workflow ... |
| `orchestrator-route` | PreToolUse Hook: Route tasks through central orchestrator for optimization. |
| `post-compact-continue` | Post-Compact Continue Hook

Triggers after conversation compaction to inject con... |
| `post-tool-use-tracker` | Post-tool-use hook that:
1. Tracks edited files and their repos (Edit/Write tool... |
| `post_tool_use_tracker` | Post-tool-use tracker - Cross-platform Python port.

Tracks:
1. Build/test attem... |
| `pre-compact-handoff` | PreCompact hook - auto-saves state before context compaction.

When context is a... |
| `pre_compact_continuity` | PreCompact Continuity Hook (Python port).
...
Cross-platform Pyth... |
| `session_start_continuity` | Session Start Continuity Hook (Python port).

Loads YAML handoff context at sess... |
| `smart-tool-redirect` | PreToolUse hook: ENFORCES smart tool usage for token optimization.
Blocks native... |
| `stop-memory-sync` | Stop hook - triggers memory sync when dirty files exist.

At end of turn, checks... |
| `unified-post-tool-tracker` | Unified Post Tool Use Tracker - Consolidates all post-tool tracking.

Replaces:
... |
| `utf-context-filter` | UTF Context Filter Hook - Taxonomy-Based Context Retrieval

PreToolUse hook that... |
| `utf-semantic-rag` | UTF Semantic RAG Hook - Token-Efficient Context Retrieval

PreToolUse hook for R... |
